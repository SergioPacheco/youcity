...
(function() {
  ...
  const elements = {
    ...
    cityGuideContent: $("#city-guide-content"),
    mapModal: $("#map-modal"),
  ...
  const state = {
    ...
    cityGuideRequestId: 0,
    cityGuideCache: new Map(),
  ...
  function updateMainTravelCta(city, hasOffers) {
    ...
    // The top action also opens the City Guide, which remains available even
    // when the affiliate catalog has no offer for the current destination.
  ...
  function renderTravelPlanner(city) {
    ...
    if (plannerTitle) plannerTitle.textContent = `Explore ${city.name}`;
  ...
  async function updateCityWeather(city) {
    ...
    try {
      ...
      if (isStaticLocalPreview()) {
        // A plain static server does not execute Pages Functions. Use the
        // public endpoint directly so local previews do not generate 404s.
        ...
        try {
          ...
          // A static preview or an older deployment may not have the Pages
          // Function. Keep the UI useful with Open-Meteo's public endpoint.
  ...
  const CITY_GUIDE_LANGUAGES = new Set(["en", "es", "pt", "fr", "de", "it"]);
  ...
  async function openCityGuide() {
    ...
    const cached = state.cityGuideCache.get(cacheKey);
    if (cached && cached.expiresAt > Date.now()) {
      renderCityGuide(cached.value, city);
      return;
    }

    const requestId = ++state.cityGuideRequestId;
    ...
    try {
      ...
      if (requestId !== state.cityGuideRequestId) return;
      if (!data?.wikipedia && !data?.places?.length) throw new Error("City guide returned no content");
      state.cityGuideCache.set(cacheKey, { value: data, expiresAt: Date.now() + 24 * 60 * 60 * 1000 });
